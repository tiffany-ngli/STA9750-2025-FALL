...
---
# INTRODUCTION
The Metropolitan Transportation Authority (MTA) remains one of the most relied on public transportation systems in New York City, [retaining high ridership levels post-pandemic across major metro areas. However, ever since COVID-19, there has been a 20% decline in ridership compared to pre-COVID ridership levels.](https://comptroller.nyc.gov/reports/nycs-post-pandemic-rebound-resilience-meets-headwinds/) Even so, daily ridership today sees up to 4.5 million riders across MTA’s subway and bus services alone. Following the economic shock, there was also a major shift in the housing market. Collectively, these changes beg the question: did the relationship between transit accessibility, ridership levels, and property values change from pre-COVID and post-COVID across NYC’s Community Districts?
...
get_nyc_cd <- function() {
  ...
  cd_sf <- st_read(shp_path, quiet = TRUE) |>
    ...
    mutate(
      ...
      boro_num = substr(sprintf("%03d", boro_cd), 1, 1), # Retrieves borough number from BoroCD
      boro_abbr = case_when( # Assigns abbreviated borough names to borough number
        boro_num == "1" ~ "MN",
        boro_num == "2" ~ "BX",
        boro_num == "3" ~ "BK",
        boro_num == "4" ~ "QN",
      ...
      cd_num = sprintf("%02d", boro_cd %% 100), # Retrieves district number from BoroCD
      cd_id = paste0(boro_abbr, cd_num) # Creates cd_id with proper naming convention
    ) |>
    filter(as.integer(cd_num) <= 18) # Keeps only 59 standard CDs
...
kable(ridership_coef_tbl_pre, caption = "Pre-COVID Linear Model")
...
kable(ridership_coef_tbl_post, caption = "Post-COVID Linear Model")
```
...
kable(accessibility_coef_tbl_pre, caption = "Pre-COVID Linear Model")
...
kable(accessibility_coef_tbl_post, caption = "Post-COVID Linear Model")
```

Analyzing the linear models above, both periods show a significant relationship between accessibility score and housing prices, where p-values were less than 0.001. However, uncertainty in the model persists with a moderate R-squared value, which also weakened post-COVID, dropping from 0.366 to 0.36. The pre-COVID model interpreted results show that a 1-point increase in accessibility score is associated with a roughly $14,824 higher median housing sale price. The slope dropped post-COVID, where a 1-point increase in accessibility score is associated with roughly $13,422. This shift likely reflects the overall increase in median housing sale prices in the post-COVID period, which reduced the impact of a 1-point increase on sale price. Another indicator of increased housing prices was the increase in intercept, where a CD with a 0 accessibility score is expected to have a median sale price of $306,162. While post-COVID, the median sale price is expected to be $442,592. To conclude, although housing prices rose post-COVID, the relationship between median sale price and transit accessibility remained significant with a positive correlation.    

### Accessibility Scores Across CDs
The map below displays the metrics involved in calculating the accessibility score assigned to each CD, along with their median sale price pre-COVID and post-COVID. Scores range from 0 to 100 after they were normalized by max ridership density and transit stop density. The score equally weighs each component, as ridership density represents demand for service, while transit stop density represents supply. Furthermore, since CD boundaries are fixed and changes in station counts are minimal, the changes in accessibility score over time are driven by changes in ridership density.
```{r, echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: true
#| code-summary: "Accessibility Score - Interactive Map"
library(leaflet)
library(sf)
library(tidyverse)
library(htmltools)
...
# MAP: TRANSIT ACCESSIBILITY SCORE WITH HOUSING PRICES
map_accessibility <- leaflet(plot_data_wgs84) %>%
  ...
  addPolygons(
    ...
    label = ~lapply(paste0(
      ...
      "<strong>Area:</strong> ", round(area_sq_miles, 2), " sq mi<br/>",
      "<hr style='margin:4px 0'/>",
      "<strong>Median Sale Price (Pre-COVID):</strong> ", 
      ifelse(is.na(median_sale_price_precovid), 
             "No data", 
             paste0("$", scales::comma(round(median_sale_price_precovid)))), "<br/>",
      "<strong>Median Sale Price (Post-COVID):</strong> ", 
      ifelse(is.na(median_sale_price_postcovid), 
             "No data", 
             paste0("$", scales::comma(round(median_sale_price_postcovid))))
    ), HTML),
  ...
  addControl(
    html = "<h4 style='margin:0'>Transit Accessibility & Housing Prices</h4>
            <p style='margin:0;font-size:12px;color:gray'>
            Hover over districts to see accessibility scores and median housing prices
            </p>",
    position = "topright"
  )
...
At the borough level, Manhattan (in green) dominates the upper clusters with high accessibility scores and high median sale prices. However, there is higher variability in this region, as shown by the shaded gray. Once again, CD MH05 with the highest accessibility score shows an indication of a potential leverage point. On the contrary, the Bronx (in red) and Staten Island (in orange) take the lower end of the axis with lower accessibility scores and median sale prices. Leaving Brooklyn (blue) and Queens (purple) casts a wider range in scores and prices.
```{r, fig.width=12, fig.height=8, dpi=300, warning=FALSE, message=FALSE}
#| code-fold: true
#| code-summary: "Show code"
...
All in all, the relationship between transit accessibility, ridership levels, and property values remains positive and statistically significant during both periods across NYC. The observable change, however, was the magnitude of accessibility and ridership, while median sale prices increased post-COVID. Additionally, their explanatory power weakened post-COVID, suggesting that transit trends alone are not a strong indicator of property values.

Comparing ridership and accessibility as individual predictors, ridership emerges as the stronger predictor with a stronger statistical fit in the model. As demonstrated by the ridership model having greater t-statistics pre-COVID (t ≈ 6.60, R-squared ≈ 0.433) and post-COVID (t ≈ 6.36, R-squared ≈ 0.415) as opposed to the accessibility model t-statistics pre-COVID (t ≈ 5.74, R-squared ≈ 0.366) and post-COVID (t ≈ 5.65, R-squared ≈ 0.360), indicating greater precision and correlation. Seeing that each coefficient had a p-value less than 0.0001 shows both predictors are highly significant in relation to price.

To conclude, it is evident that transit accessibility and ridership are statistically significant factors in driving property values. Even so, post-COVID led to a multitude of diverse changes in the economy, migration patterns, and people’s lifestyles. Although this study gives an in-depth analysis of the relationship between transit trends and property values, the models are too simple to capture the complex interactions to complete a holistic model in predicting property values. Seemingly, transit trends are significant predictors; it’s worthwhile to observe how it performs in a multivariate regression model, with other predictors of property value. 
