...
It’s important to note that in **2003**, the BLS redesigned the Current Employment Statistics (CES) survey, causing missing values for that year.
```{r, warning=FALSE}
#| code-fold: true
#| code-summary: "Show the code"
#1. What and when were the largest revisions (positive and negative) in CES history?

#CALCULATE YEARLY REVISIONS BY GROUPING BY YEAR, THEN TAKING THE MEAN
yearly_mean_revisions <- joined_ces |>
  group_by(year = year(date)) |>
  mutate(revision = as.numeric(revision)) |>
  summarize(mean_revision = mean(revision))

#IDENTIFIES THE MIN AND MAX OF MEAN REVISIONS 
...
#GRAPH HIGHLIGHTS THE MIN AND MAX POINTS TO IDENTIFY THE YEARS IT OCCURRED IN 
...
#SORT POSITIVE REVISIONS FROM NEGATIVE REVISIONS
...
#BAR GRAPH TO SHOW THE DIFFERENCE
...
#CALCULATE MAGNITUDE BY TAKING THE ABSOLUTE VALUE OF REVISIONS
revision_magnitude <- joined_ces |>
  mutate(magnitude = abs(as.numeric(revision)),
         revision = as.numeric(revision)) 

#HIGHLIGHTS IN GRAY THE PERIODS THAT SHOWED THE MOST FLUCTUATION
...
#CALCULATE REVISION PERCENTAGE BY DIVIDING MAGNITUDE (PART) OVER LEVEL (WHOLE)
percent_employment <- revision_magnitude %>%
  mutate(
    "revision percentage"= abs(revision) / level * 100
  )

#PLOTS THE REVISION PERCENTAGE WITH THE THRESHOLD 0.2 INDICATING HIGH REVISION PERCENTAGE
ggplot(percent_employment, aes(x = date, y = `revision percentage`)) +
  geom_line(color = "steelblue") +
  labs(
    title = "Revision Percentage of Employment Over Time",
    x = "Date",
...
#IDENTIFIES THE MAX AND MIN REVISION PERCENTAGES
...
#CALCULATES AVERAGE MONTHLY REVISIONS BY GROUPING BY MONTH, THEN TAKING MEAN REVISIONS
...
#PLOTS ABSOLUTE MEAN REVISIONS FOR EACH MONTH HIGHLIGHTING THE MIN AND MAX REVISIONS
...
#OBSERVES AVERAGES OF REVISION MAGNITUDE AND REVISION PERCENTAGES
revision_summary <- joined_ces |>
  ...
  summarize(
    "Average Magnitude Revision" = mean(abs_revision, na.rm = TRUE), 
...
#CALCULATES YEARLY NEGATIVE FRACTIONS
...
#SEPARATES POST 2000 AND PRE 2000 
...
#PLOTS THE DIFFERENCE IN NEGATIVE FRACTIONS
...
#PERFORMS T-TEST
#NULL HYPOTHESIS: THERE IS NO DIFFERENCE BETWEEN PRE-2000 AND POST-2000 NEGATIVE FRACTIONS
#ALTERNATIVE HYPTHESIS: THERE IS A DIFFERENCE BETWEEN PRE-2000 AND POST-2000 NEGATIVE FRACTIONS
...
#SEPARATED BY PRE 2020 AND POST 2020
...
#RUN T-TEST
#NULL HYPOTHESIS: THERE IS NO DIFFERENCE BETWEEN PRE 2020 AND POST 2020 AVERAGE YEARLY REVISIONS
#ALTERNATIVE HYPOTHESIS: THERE IS A DIFFERENCE BETWEEN PRE 2020 AND POST 2020 AVERAGE YEARLY REVISIONS
...
#LOADS IN PRESIDENTS PARTY AND TERM PERIODS
presidents_party <- expand_grid(year=1979:2025, 
                                       month = month.name, 
                                       president = NA, 
                                       party = NA) |> 
...
#FILTERS BIDEN AND TRUMP'S CURRENT PRESIDENCY
claim1 <- presidents_party |>
  filter(president == c("Biden", "Trump II")) 


#ADDS MONTH AND YEAR COLUMNS INDIVUALLY USED TO JOIN TABLES 
claim1_percentage_employment <- percent_employment |>
  mutate(month = month(date, label = TRUE, abbr = FALSE),
         year = year(date))

#JOINS REVISION PERCENTS TO BIDEN AND TRUMP'S TERMS 
claim1 <- claim1 |>
  left_join(claim1_percentage_employment, by=c("year", "month"))

#IDENTIFIES THE TERMS FOR EACH PRESIDENT
...
ggplot(claim1, aes(x = date, y = `revision percentage`)) +
  ...
  # POINTS FOR EACH MONTH'S OBSERVATION
  geom_point(aes(color = party), size = 2) +
  
  # HIGHLIGHTS NOVEMBER 2024 -- AS STATED BY TRUMP
...
# FILTERS ALL PREVIOUS NOVEMBERS (EXCLUDING NOVEMBER 2024)
...
#ONE-SAMPLE T-TEST: NOV 2024 > HISTORICAL MEAN
...
#FILTERS TRUMP'S MOST CURRENT TERM
trump <- claim1 |>
...
#RETRIVES TRUMP'S FIRST AND SECOND TERM
...
#FILTERS BIDEN'S TERM
biden <- claim1 |>
  filter(president == "Biden")

#PLOTS PAYROLL DURING BIDEN'S TERM
